"""
LightGBM final model training module for Google Stock ML project.
Matches notebook Cell 64 (BLOCK 25B) exactly.
Identical structure to xgboost_model.py.
"""
...
# ==============================================================================
# CELL 64 (BLOCK 25B): LIGHTGBM FINAL MODEL
# ==============================================================================

def split_valid_es_score_lgb(Xv, yv, wv, valid_es_start, valid_es_end, valid_score_start, valid_score_end):
    """
    Split validation into ES (early stopping) and SCORE (model selection) sets.
    Matches notebook split_valid_es_score_lgb() exactly.
    """
...
def get_default_lgb_params(config: Dict) -> Dict:
    """
    Get default LightGBM params from config.
    Used when best_params_lgb file is not found.
    """
    return {
        ...
        "subsample": float(config.get("subsample", 0.7)),
        "colsample_bytree": float(config.get("colsample_bytree", 0.7)),
        "reg_alpha": float(config.get("reg_alpha", 1e-4)),
        "reg_lambda": float(config.get("reg_lambda", 5.0)),
    }


def train_final_model_lgb(
    ...
    pred_lgb_local: Optional[Path] = None,
    pred_lgb_drive: Optional[Path] = None,
    proc_data_local: Optional[Path] = None,
    proc_data_drive: Optional[Path] = None
) -> Optional[Dict[str, Any]]:
    """
    Train final LightGBM model and evaluate.
    Matches notebook Cell 64 (BLOCK 25B) exactly.
    Identical structure to train_final_model() in xgboost_model.py.
    ...
    Saves to outputs_local/outputs_drive:
        - baseline_zero_lgb_results.json
        - baseline_naive_lgb_results.json
    
    Saves to pred_lgb_local/pred_lgb_drive:
    ...
    Returns:
        Dict with model, predictions, metrics, baselines (or None if LightGBM not available)
    """
    ...
    try:
        ...
        print("[SKIP] BLOCK 25B — LightGBM not installed")
        return None
    ...
    print(f"[INFO] Loaded: X_train_sel={X_train.shape} | X_valid_sel={X_valid.shape} | X_test_sel={X_test.shape}")
    ...
    for key in ["n_estimators", "objective", "metric", "random_state", "n_jobs", "verbose"]:
    ...
    if outputs_local:
        ...
        for baseline_row in baseline_results:
            if baseline_row["split"] == "TEST":
                ...
                baseline_name = baseline_row["model"].lower() + "_lgb"
    ...
    # Predictions
    if pred_lgb_local:
        pred_lgb_local = ensure_dir(Path(pred_lgb_local))
        ...
        if pred_lgb_drive:
            pred_lgb_drive = ensure_dir(Path(pred_lgb_drive))
    ...
    if pred_lgb_local:
        ...
        try:
            ...
            ax.set_title(f"LightGBM Predictions — xgb_selected — last {len(hist_tail)} days + tomorrow")
            ax.set_xlabel("Date")
    ...
    if SHAP_ENABLED and pred_lgb_local:
        ...
        try:
            ...
            # Create explainer (TreeExplainer works for LightGBM too)
            ...
            if SHAP_SAVE_VALUES:
                ...
                shap_df.to_csv(pred_lgb_local / "shap_values_test.csv")
                if pred_lgb_drive:
                    copy_file(pred_lgb_local / "shap_values_test.csv", pred_lgb_drive / "shap_values_test.csv")
            ...
            shap_importance.to_csv(pred_lgb_local / "shap_feature_importance.csv", index=False)
            if pred_lgb_drive:
                copy_file(pred_lgb_local / "shap_feature_importance.csv", pred_lgb_drive / "shap_feature_importance.csv")
            ...
            if SHAP_BAR:
                ...
                plt.tight_layout()
                plt.savefig(pred_lgb_local / "shap_summary_bar.png", dpi=DPI, bbox_inches="tight")
                if pred_lgb_drive:
                    copy_file(pred_lgb_local / "shap_summary_bar.png", pred_lgb_drive / "shap_summary_bar.png")
                plt.close()
            ...
            if SHAP_BEESWARM:
                ...
                plt.tight_layout()
                plt.savefig(pred_lgb_local / "shap_summary_beeswarm.png", dpi=DPI, bbox_inches="tight")
                if pred_lgb_drive:
                    copy_file(pred_lgb_local / "shap_summary_beeswarm.png", pred_lgb_drive / "shap_summary_beeswarm.png")
                plt.close()
